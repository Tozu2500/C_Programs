...
void sort_students_by_name(Student students[], int count) {
    ...
    for (int i = 0; i < count - 1; i++) {
        for (int j = 0; j < count - i; j++) {
            ...
            if (cmp > 0 || (cmp == 0 && strcasecmp(students[i].first_name, students[j + 1].first_name) > 0)) {
...
void sort_students_by_gpa(Student students[], int count) {
    ...
    // Bubble sort for gpa, descending order
    for (int i = 0; i < count - 1; i++) {
        for (int j = 0; j < count - i - 1; j++) {
...
void generate_class_report(StudentDatabase *db, const char *course) {
    ...
    if (count == 0) {
        printf("No students found for this course: %s\n", course);
        return;
    }
    ...
    if (students_with_grades > 0) {
        ...
        printf("No grades recorded from the database for any student in this particular course\n");
    }
