...
console_t* console_init(void) {
    ...
    SetConsoleTitle("Web Scraper v1.0");
...
void console_clear(console_t* console) {
    ...
    FillConsoleOutputAttribute(console->hConsole, console->csbi.wAttributes, size, coord, &written);
...
int console_get_number(console_t* console, const char* prompt, int min, int max) {
    ...
    while (1) {
        ...
        console_print_colored(console, COLOR_RED, COLOR_BLACK, 
    ...
    while (1) {
        ...
        for (int i = 0; i < menu->item_count; i++) {
            if (i == menu->selected_index) {
                console_print_colored(console, COLOR_BLACK, COLOR_WHITE, 
                                    " > %s\n", menu->items[i].text);
            } else {
